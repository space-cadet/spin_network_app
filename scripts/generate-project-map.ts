...
function generateProjectMapMarkdown(items: ProjectItem[]): string {
  ...
  const topLevelDirs = items
    .filter(item => item.isDirectory && !item.path.includes('/'))
    .sort((a, b) => a.path.localeCompare(b.path));
  
  topLevelDirs.forEach(dir => {
    markdown += `- [${dir.path}](#${dir.path})\n`;
  });
  markdown += `- [Root Files](#root-files)\n\n`;

  // Function to generate indentation
  const getIndent = (depth: number) => '  '.repeat(depth);

  // Function to format a single item
  const formatItem = (item: ProjectItem, depth: number) => {
    const indent = getIndent(depth);
    const icon = item.isDirectory ? '📁' : '📄';
    const name = path.basename(item.path);
    const typeDisplay = item.type.padEnd(15);
    const descDisplay = item.description ? ` - ${item.description}` : '';
    return `${indent}${icon} \`${name}\` *(${typeDisplay})* ${descDisplay}\n`;
  };

  // Process each top-level directory
  topLevelDirs.forEach(topDir => {
    markdown += `## ${topDir.path}\n\n`;
    if (topDir.description) {
      markdown += `${topDir.description}\n\n`;
    }

    // Get all items in this directory
    const dirItems = items
      .filter(item => item.path.startsWith(topDir.path + '/'))
      .sort((a, b) => {
    ...
    dirItems.forEach(item => {
      ...
      markdown += formatItem(item, depth - 1);
    });
    markdown += '\n';
  });
  ...
  markdown += `- Types are inferred based on location and extension.\n`;
  markdown += `- This file is auto-generated. Do not edit manually.\n`;
  markdown += `- Navigate using the Table of Contents above.\n`;
