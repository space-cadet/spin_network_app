...
describe('Angular Momentum Operators', () => {
    ...
    it('should satisfy [Jx,Jy] = iJz', () => {
      ...
      for (let i = 0; i < matrix1.length; i++) {
        for (let j = 0; j < matrix1[i].length; j++) {
          expect(math.abs(math.subtract(matrix1[i][j], matrix2[i][j])) < 1e-10).toBe(true);
        }
      }
    });
  });
  ...
  describe('Expectation Values', () => {
    ...
    it('should compute correct Jz expectation values', () => {
      ...
      expect(math.abs(math.subtract(expectUp, math.complex(1/2, 0))) < 1e-10).toBe(true);
      expect(math.abs(math.subtract(expectDown, math.complex(-1/2, 0))) < 1e-10).toBe(true);
    });
  });
});
