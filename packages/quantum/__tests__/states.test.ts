...
describe('Quantum States', () => {
  describe('computationalBasis', () => {
    it('creates valid basis for single qubit', () => {
      ...
      // |0⟩ state
      expect(basis[0].amplitudes[0]).toEqual(math.complex({re: 1, im:  0}));
      expect(basis[0].amplitudes[1]).toEqual(math.complex({re: 0, im:  0}));
      
      // |1⟩ state
      expect(basis[1].amplitudes[0]).toEqual(math.complex({re: 0, im:  0}));
      expect(basis[1].amplitudes[1]).toEqual(math.complex({re: 1, im:  0}));
    });
  ...
  describe('createBasisState', () => {
    it('creates valid computational basis states', () => {
      ...
      expect(state0.amplitudes[0]).toEqual(math.complex({re: 1, im:  0}));
      expect(state0.amplitudes[1]).toEqual(math.complex({re: 0, im:  0}));
      expectStateNormalized(state0);

      const state1 = createBasisState(2, 1);
      expect(state1.amplitudes[0]).toEqual(math.complex({re: 0, im:  0}));
      expect(state1.amplitudes[1]).toEqual(math.complex({re: 1, im:  0}));
  ...
  describe('Bell States', () => {
    it('creates normalized Phi+ Bell state', () => {
      ...
      expect(state.amplitudes[0]).toEqual(math.complex({re: 1/Math.sqrt(2), im:  0}));
      expect(state.amplitudes[1]).toEqual(math.complex({re: 0, im:  0}));
      expect(state.amplitudes[2]).toEqual(math.complex({re: 0, im:  0}));
      expect(state.amplitudes[3]).toEqual(math.complex({re: 1/Math.sqrt(2), im:  0}));
    });
    ...
    it('creates normalized Psi+ Bell state', () => {
      ...
      expect(state.amplitudes[1]).toEqual(math.complex({re: 1/Math.sqrt(2), im:  0}));
      expect(state.amplitudes[2]).toEqual(math.complex({re: 1/Math.sqrt(2), im:  0}));
    });
  ...
  describe('GHZ States', () => {
    ...
    it('creates valid 3-qubit GHZ state', () => {
      ...
      expect(state.amplitudes[0]).toEqual(math.complex({re: 1/Math.sqrt(2), im:  0}));
      expect(state.amplitudes[7]).toEqual(math.complex({re: 1/Math.sqrt(2), im:  0}));
    });
  ...
  describe('W States', () => {
    ...
    it('creates valid 3-qubit W state', () => {
      ...
      const expectedAmp = math.complex({re: 1/Math.sqrt(3), im:  0});
      expect(state.amplitudes[1]).toEqual(expectedAmp);
      expect(state.amplitudes[2]).toEqual(expectedAmp);
